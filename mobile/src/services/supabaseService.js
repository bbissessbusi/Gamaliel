...
if (!supabaseUrl || !supabaseAnonKey) {
  console.warn(
    'Supabase credentials not found. Set EXPO_PUBLIC_SUPABASE_URL and EXPO_PUBLIC_SUPABASE_ANON_KEY in mobile/.env'
  );
}
...
// Exposed for direct REST uploads (progress-tracked XHR upload)
...
/**
 * Sign in (or sign up) with Google or Apple.
 *
 * NOT YET WIRED UP: native OAuth needs expo-auth-session (Google) and
 * expo-apple-authentication (Apple Sign In entitlement), plus your OAuth
 * client IDs configured in Supabase. The UI is feature-flagged off via
 * OAUTH_ENABLED in LoginPage/SignUpPage until that's provided — see
 * mobile/README.md.
 */
export async function signInWithOAuth(_provider) {
  throw new Error('OAuth sign-in is not configured yet for the mobile app.');
}
...
export async function saveEvaluation(data) {
  if (!supabase) throw new Error('Supabase not configured. Add credentials to mobile/.env');
